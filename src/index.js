...
function carregarDados() {
  if (fs.existsSync(dbFile)) {
    try {
      ...
      return {
        configServidor: data.configServidor || {}, // guildId -> { pixKey, pixQrCode, pixReceiverName, logChannelId }
        produtos: data.produtos || {},             // id -> produto obj
        paineis: data.paineis || {},               // panelId -> painel obj
        carrinhosAtivos: data.carrinhosAtivos || {}, // userId -> carrinho obj
        cupons: data.cupons || {},                 // nome -> cupom obj
        conexoesVoz: data.conexoesVoz || {}          // guildId -> conexao obj
      };
    } catch (e) {
      console.error('Erro ao ler banco de dados:', e);
    }
  }
  return { configServidor: {}, produtos: {}, painel: {}, carrinhosAtivos: {}, cupons: {}, conexoesVoz: {} };
}

function salvarDados() {
  try {
    const data = { configServidor, produtos, painel, carrinhosAtivos, cupons, conexoesVoz };
...
const painel = dadosSalvos.paineis || dadosSalvos.painel || {};
...
const aguardandoUploadPix = new Map(); // userId -> guildId
...
client.on('interactionCreate', async (interaction) => {
  try {
    ...
    if (interaction.isButton()) {
      ...
      if (cid === 'carrinho_cancelar') {
        ...
        if (cartKey) {
          ...
          if (prod) {
            prod.estoque += carrinho.quantity; // devolve estoque reservado
            salvarDados();
          }
      ...
      if (cid.startsWith('comprar_btn_')) {
        ...
        // Decrementa estoque preventivamente
        prod.estoque -= 1;
        salvarDados();
    ...
    if (interaction.isStringSelectMenu()) {
      ...
      // MENU DE COMPRA DE PAINEL PERSISTENTE
