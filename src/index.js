...
client.on('interactionCreate', async (interaction) => {
  try {
    ...
    if (interaction.isButton()) {
      ...
      // Tratamento de botões de compra diretos do painel se houver
      if (interaction.customId.startsWith('comprar_btn_')) {
        ...
        if (!prod || prod.estoque <= 0) {
          return interaction.reply({ embeds: [aviso('Produto Indisponível', '⚠️ Este produto acabou de ficar sem estoque.')], ephemeral: true });
        }
        ...
        const embedCarrinho = new EmbedBuilder()
          ...
          .setDescription(
            `Produto: ${prod.nome}\n` +
            `Quantidade: 1\n` +
            `Valor unitário: ${formatarMoeda(prod.preco)}\n` +
            `Total: ${formatarMoeda(prod.preco)}`
          );

        const rowCarrinho = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId(`carrinho_pagar_${prodId}`).setLabel('Pagar').setStyle(ButtonStyle.Success).setEmoji('💳'),
          new ButtonBuilder().setCustomId('carrinho_cancelar').setLabel('Cancelar').setStyle(ButtonStyle.Danger).setEmoji('🗑️')
        );

        return interaction.editReply({ embeds: [embedCarrinho], components: [rowCarrinho] });
      }

      if (interaction.customId === 'carrinho_cancelar') {
        return interaction.update({ content: '❌ Carrinho cancelado.', embeds: [], components: [] }).catch(() => {});
      }

      if (interaction.customId.startsWith('carrinho_pagar_')) {
        return interaction.update({ content: '💳 Processando pagamento... (Simulação concluída com sucesso!)', embeds: [], components: [] }).catch(() => {});
      }
      ...
      if (interaction.customId === 'p_sel_produtos') {
        ...
        const options = prodKeys.slice(0, 25).map(id => {
          ...
          return {
            ...
            description: disponivel ? `💰 Valor: ${formatarMoeda(prod.preco)} | 📦 Estoque: ${prod.estoque} unidades` : `❌ Indisponível no momento`,
            value: id,
            emoji: prod.emoji !== 'Não definido' ? prod.emoji : '📦'
          };
        });
        ...
        if (pData.modoExibicao === 'menu' && pData.produtosSelecionados && pData.produtosSelecionados.length > 0) {
          const options = pData.produtosSelecionados.map(id => {
            ...
            return {
              ...
              value: id,
              description: disponivel ? `💰 Valor: ${formatarMoeda(prod.preco)} | 📦 Estoque: ${prod.estoque} unidades` : `❌ Indisponível no momento`,
              emoji: prod && prod.emoji !== 'Não definido' ? prod.emoji : '📦'
            };
          });
          ...
          pData.produtosSelecionados.slice(0, 5).forEach(id => {
            ...
            if (prod) {
              rowBtn.addComponents(
                new ButtonBuilder().setCustomId(`comprar_btn_${id}`).setLabel(prod.nome).setStyle(prod.estoque > 0 ? ButtonStyle.Success : ButtonStyle.Secondary).setEmoji(prod.emoji !== 'Não definido' ? prod.emoji : '🛒')
              );
            }
          });
    ...
    if (interaction.isStringSelectMenu()) {
      ...
      if (interaction.customId === 'comprar_produto_menu') {
        ...
        // Verificação prévia de estoque antes de abrir o carrinho
        if (!prod || prod.estoque <= 0) {
          return interaction.followUp({ embeds: [aviso('Produto Indisponível', '⚠️ Este produto acabou de ficar sem estoque.')], ephemeral: true });
        }
        ...
        return interaction.followUp({ embeds: [embedCarrinho], components: [rowCarrinho], ephemeral: true });
      }
