...
client.on('interactionCreate', async (interaction) => {
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (commandName === 'reestock') {
        const embed = new EmbedBuilder()
          ...
          .setDescription('Clique no botão abaixo para enviar sua solicitação de reestoque.');

        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId('btn_solicitar_estoque').setLabel('Solicitar Produto').setStyle(ButtonStyle.Primary).setEmoji('🛒')
        );
      ...
      if (commandName === 'dashboard') {
        const embed = sucesso('Dashboard do Bot', 'Métricas e Estatísticas')
          .addFields(
      ...
      if (commandName === 'cupom') {
        return interaction.reply({ embeds: [sucesso('Sistema de Cupons', 'Gerenciamento de cupons ativo.')], ephemeral: true });
      }

      // /conectar
      if (commandName === 'conectar') {
        if (!interaction.member.voice.channel) {
          return interaction.reply({ embeds: [erro('Erro', 'Você precisa estar em um canal de voz!')], ephemeral: true });
        }
        return interaction.reply({ embeds: [sucesso('Conectado', 'Canal de voz vinculado com sucesso.')], ephemeral: true });
      }
    }
    ...
    if (interaction.isButton()) {
      if (interaction.customId === 'prod_emoji') {
        const modal = new ModalBuilder()
          .setCustomId('modal_prod_emoji')
          .setTitle('Definir Emoji do Produto');

        const inputEmoji = new TextInputBuilder()
          .setCustomId('emoji_valor')
          .setLabel('Cole o emoji ou emoji personalizado')
      ...
      if (interaction.customId === 'comprar_produto') {
        ...
        const channelName = `🛒-carrinho-${interaction.user.username}`.toLowerCase().replace(/[^a-z0-9-]/g, '');
        ...
        return interaction.reply({ embeds: [sucesso('Carrinho Criado', `Seu carrinho foi aberto com sucesso em ${channel}!`)], ephemeral: true });
      }

      if (interaction.customId === 'btn_solicitar_estoque') {
        const modal = new ModalBuilder()
          .setCustomId('modal_reestock')
        ...
        const descProd = new TextInputBuilder()
          .setCustomId('reest_desc')
          .setLabel('Descrição (Opcional)')
          .setStyle(TextInputStyle.Paragraph)
          .setRequired(false);

        modal.addComponents(new ActionRowBuilder().addComponents(nomeProd), new ActionRowBuilder().addComponents(descProd));
        return interaction.showModal(modal);
      }
    }
    ...
    if (interaction.isModalSubmit()) {
      if (interaction.customId === 'modal_prod_emoji') {
        const emoji = interaction.fields.getTextInputValue('emoji_valor');
        return interaction.reply({ embeds: [sucesso('Emoji Definido', `O emoji do produto foi alterado para: ${emoji}`)], ephemeral: true });
      }

      if (interaction.customId === 'modal_reestock') {
        const nome = interaction.fields.getTextInputValue('reest_nome');
        return interaction.reply({ embeds: [sucesso('Solicitação Enviada', `Sua solicitação para o produto **${nome}** foi enviada à administração!`)], ephemeral: true });
      }
    }
    ...
    if (!interaction.replied && !interaction.deferred) {
      await interaction.reply({ embeds: [erro('Erro', 'Ocorreu um erro ao executar esta ação.')], ephemeral: true }).catch(() => {});
    }
  }
});
