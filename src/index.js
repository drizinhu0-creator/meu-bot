    ...
    if (interaction.isButton()) {
      if (['prod_nome', 'prod_preco', 'prod_estoque', 'prod_emoji', 'prod_desc', 'prod_categoria'].includes(interaction.customId)) {
        const mapModal = {
          ...
          prod_desc: { id: 'modal_p_desc', title: 'Descrição do Produto', label: 'Descrição detalhada', placeholder: 'Ex: Teste 1' },
      ...
      if (['p_alt_titulo', 'p_alt_desc', 'p_alt_cor'].includes(interaction.customId)) {
        const mapPainelModal = {
          ...
          p_alt_desc: { id: 'modal_painel_desc', title: 'Alterar Descrição', label: 'Descrição', placeholder: 'Teste 1' },
      ...
      if (interaction.customId.startsWith('comprar_btn_')) {
        ...
        // Verificar se usuário já possui carrinho aberto
        ...
        await canalCarrinho.send({ content: `${interaction.user}`, embeds: [embedCarrinho], components: [rowCarrinho] });
      ...
      if (interaction.customId.startsWith('carrinho_pagar_')) {
        delete carrinhosAtivos[interaction.user.id];
        salvarDados();
        await interaction.update({ content: '💳 Pagamento processado com sucesso! Obrigado pela compra.', embeds: [], components: [] }).catch(() => {});
      ...
      if (interaction.customId === 'p_sel_produtos') {
        ...
        const options = prodKeys.slice(0, 25).map(id => {
          ...
          return {
            ...
            description: disponivel ? `💰 Valor: ${formatarMoeda(prod.preco)} | 📦 Estoque: ${prod.estoque}` : `❌ Indisponível no momento`,
            value: id,
            emoji: '📦'
          };
        });
