  ...
  if (fs.existsSync(dbFile)) {
    try {
      ...
      return {
        ...
        ticketsConfig: data.ticketsConfig || {} // { setores: {}, painel: { titulo, desc, bannerPath, horarios: [] }, cargos: [] }
      };
    } catch (e) {
...
const cupomTemp = new Map();   
const ticketTemp = new Map();  // userId -> dados temporários de edição de ticket/painel
...
// Funções de interface em tempo real
...
// ==========================================
// 6. MANIPULAÇÃO DE INTERAÇÕES (ANTI-TIMEOUT)
// ==========================================
client.on('interactionCreate', async (interaction) => {
  try {
    ...
    if (interaction.isButton()) {
      // Evita falhas de timeout respondendo imediatamente
      if (!interaction.deferred && !interaction.replied) {
        await interaction.deferUpdate().catch(() => {});
      }

      // Painel Modo de Vendas
      ...
      // ==========================================
      // SISTEMA DE TICKETS COMPLETO (EXIGÊNCIA NOVA)
      // ==========================================
      ...
      if (interaction.customId === 't_setor_criar') {
        ...
        modal.addComponents(
          new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('s_id').setLabel('ID Único (ex: suporte, compras)').setStyle(TextInputStyle.Short).setRequired(true)),
      ...
      // Interações dentro de canais de ticket abertos
    ...
    if (interaction.isStringSelectMenu()) {
      ...
      // Abertura de Ticket pelo Painel Publicado
