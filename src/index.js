...
let reestockCooldownMinutos = 5;  // Cooldown padrão em minutos (configurável pela engrenagem)
  ...
  try {
    ...
    if (interaction.isButton()) {
      if (interaction.customId === 'config_canal_solicitacao') {
        ...
        return interaction.reply({ content: 'Selecione o canal de **Solicitações** (onde a staff vai aprovar/rejeitar os pedidos):', components: [menu], ephemeral: true });
      }
      ...
      if (interaction.customId === 'config_canal_log') {
        ...
        return interaction.reply({ content: 'Selecione o canal de **Logs** (onde aparecerá o anúncio de reestock aprovado):', components: [menu], ephemeral: true });
      }
      ...
      if (interaction.customId === 'config_enviar_painel') {
        ...
        return interaction.reply({ content: 'Selecione abaixo em qual canal deseja **publicar o painel de solicitação** para os membros:', components: [menu], ephemeral: true });
      }

      // Membro clica no painel público para abrir o formulário de pedido
      ...
      // Staff clica no botão ⚙️ de CONFIGURAÇÕES da solicitação
      if (interaction.customId.startsWith('config_reest_')) {
        const solicitanteId = interaction.customId.split('_')[2];

        const rowConfig = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId(`blacklist_add_${solicitanteId}`).setLabel('Bloquear Membro (Blacklist)').setStyle(ButtonStyle.Danger).setEmoji('🚫'),
          new ButtonBuilder().setCustomId('set_cooldown_modal').setLabel('Definir Cooldown').setStyle(ButtonStyle.Secondary).setEmoji('⏳')
        );

        return interaction.reply({ content: `⚙️ **Painel de Configuração do Reestock**\nEscolha uma das ações abaixo para gerenciar este usuário ou o sistema:`, components: [rowConfig], ephemeral: true });
      }

      // Ação de adicionar usuário à Blacklist
      if (interaction.customId.startsWith('blacklist_add_')) {
        const userId = interaction.customId.split('_')[2];
        blacklistUsers.add(userId);
        return interaction.update({ content: `🚫 O usuário <@${userId}> foi adicionado à **blacklist** e não poderá mais enviar solicitações de reestock.`, components: [] });
      }

      // Abre Modal para definir o Cooldown
    ...
    if (interaction.isModalSubmit()) {
      if (interaction.customId === 'modal_enviar_reestock') {
        ...
        // 3 botões: Aprovar, Recusar e Configurações (⚙️)
        const row = new ActionRowBuilder().addComponents(
